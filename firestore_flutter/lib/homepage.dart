      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          IconButton(
            ...
            onPressed: () {
              showSearch(context: context, delegate: ItemSearch(sdSnapshot));
            },
          ),
          IconButton(
            icon: Icon(Icons.add),
...
class ItemSearch extends SearchDelegate<List<DocumentSnapshot>> {
  ...
  Widget buildSuggestions(BuildContext context) {
    final results =
        items.where((a) => a['title'].toLowerCase().contains(query)).toList();

    return ListView.builder(
        itemCount: results.length,
        itemBuilder: (context, index) {
          DocumentSnapshot snapshot = results[index];
          return CustomCard(snapshot);
        });
  }
}
